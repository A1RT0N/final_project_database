...
SELECT
  ...
  CASE WHEN i % 3 = 0 THEN '11900000'||lpad(i::text,3,'0') ELSE '11910000'||lpad(i::text,3,'0') END
FROM generate_series(0,100) AS s(i);
...
SELECT
  'D'||lpad((i % 101)::text,3,'0'),
  'C'||lpad((i % 101)::text,3,'0')
FROM generate_series(0,100) AS s(i);
...
JOIN Disciplina d 
  ON d.Codigo = 'D'||lpad((s.i)::text,3,'0');
